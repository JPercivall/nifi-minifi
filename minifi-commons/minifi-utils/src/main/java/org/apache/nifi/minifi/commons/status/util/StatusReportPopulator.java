...
public class StatusReportPopulator {
    ...
    public static void addExpectedRemoteProcessingGroupStatus(FlowStatusReport flowStatusReport, boolean addHealth, boolean addAuthorizationIssues, boolean addInputPort,
                                                              boolean addStats, boolean addBulletins, boolean populateBulletins) {
        RemoteProcessingGroupStatusBean expectedRemoteProcessingGroupStatus = new RemoteProcessingGroupStatusBean();
        expectedRemoteProcessingGroupStatus.setName("rpg1");
        if (addAuthorizationIssues) {
            expectedRemoteProcessingGroupStatus.setAuthorizationIssues(Collections.singletonList("auth issue"));
        }

        if (addHealth) {
            RemoteProcessingGroupHealth remoteProcessingGroupHealth = new RemoteProcessingGroupHealth();
            remoteProcessingGroupHealth.setTransmissionStatus("Transmitting");
            remoteProcessingGroupHealth.setHasBulletins(populateBulletins);
            remoteProcessingGroupHealth.setHasAuthorizationIssues(addAuthorizationIssues);
            remoteProcessingGroupHealth.setActivePortCount(1);
            remoteProcessingGroupHealth.setInactivePortCount(2);
            expectedRemoteProcessingGroupStatus.setRemoteProcessingGroupHealth(remoteProcessingGroupHealth);
        }
        ...
        if (addBulletins) {
            if (populateBulletins) {
                ...
                bulletinStatus.setMessage("Bulletin message");
                bulletinStatus.setTimestamp(new Date(1464019245000L));
                expectedRemoteProcessingGroupStatus.setBulletinList(Collections.singletonList(bulletinStatus));
            } else {
                expectedRemoteProcessingGroupStatus.setBulletinList(Collections.EMPTY_LIST);
            }
        }
        ...
        if (addInputPort) {
            ...
            expectedRemoteProcessingGroupStatus.setInputPortStatusList(Collections.singletonList(expectedInputPortStatus));
        }

        if (addStats) {
            RemoteProcessingGroupStats remoteProcessingGroupStats = new RemoteProcessingGroupStats();
            remoteProcessingGroupStats.setActiveThreads(3);
            remoteProcessingGroupStats.setSentContentSize(4L);
            remoteProcessingGroupStats.setSentCount(5);
            expectedRemoteProcessingGroupStatus.setRemoteProcessingGroupStats(remoteProcessingGroupStats);
        }

        flowStatusReport.setRemoteProcessingGroupStatusList(Collections.singletonList(expectedRemoteProcessingGroupStatus));
    }
